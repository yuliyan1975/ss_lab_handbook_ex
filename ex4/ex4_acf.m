...
s=repmat(s,1,cy);       % Repeat
                        % Time
t=linspace(0,cy*T-dt,length(s));
n=randn(1,length(t));   % Gaussian noise
                        % Rectangular cyclic (5 periods) + noise
% s=n;                  % Uncomment for WGN only 
% s=s+n;                % Uncomment to add WGN to the signal

r=xcorr(s,s)*dt;        % ACF
                        % ACF argument (tau). From (-length/2 to length/2)*dt
tau=linspace(-length(r)/2,length(r)/2,length(r))*dt;
